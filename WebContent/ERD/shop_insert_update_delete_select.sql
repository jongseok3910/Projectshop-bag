...
--userinfo update
update  userinfo set password='8888',name='김경호변경',email='change@email.com' where userid='guard1';
--userinfo delete
...
commit;
--cat제품존재여부 
select  count(*) from cart where userid='guard1' and p_no=4;
select  count(*) from cart where userid='guard2' and p_no=4;



-- 로그인한유저 cart에 있는 제품 1개   select(유저정보,제품정보)
...
-- 로그인한유저 cart에 있는 제품 전체  select(유저정보,제품정보)

select * from cart  where userid='guard1';

-- 로그인한유저 cart에 있는 제품 전체     delete
--delete from cart where userid='guard1';
--ROLLBACK;
-- 로그인한유저 cart에 있는 제품품목한개  delete
